...
export default function SellerAddProductPage() {
  ...
  const validate = () => {
    if (!form.name || !form.categoryId || !form.basePrice || form.images.length === 0) {
      toast.error("Please fill in all required fields (Name, Category, Price, Images)");
      return false;
    }
    return true;
  };
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <select
                    className="w-full px-6 py-4 bg-gray-50 rounded-2xl border-2 border-transparent focus:border-black outline-none font-bold appearance-none"
                ...
                {form.categoryId && getSubCategories(form.categoryId).length > 0 && (
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      {getSubCategories(form.categoryId).map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
                  </div>
                )}
              </div>
