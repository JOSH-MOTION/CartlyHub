        ...
        <div className="flex flex-col gap-2 mb-4">
          ...
          {product.sellerName && (
            <a 
              ...
              className="flex items-center text-[10px] font-black text-emerald-600 uppercase tracking-widest hover:text-black transition-colors"
        ...
        <div className="flex items-center mb-4">
          <div className="flex gap-0.5 text-yellow-400">
            {[1, 2, 3, 4, 5].map((s) => (
              <Star 
                key={s} 
                className={`h-3.5 w-3.5 ${s <= (product.averageRating || 0) ? 'fill-current' : 'text-gray-200'}`} 
              />
            ))}
          </div>
